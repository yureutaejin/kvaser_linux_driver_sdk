...
static void crc32Update(const uint8_t *pData, uint32_t uSize)
{
    unsigned long i = 0;

    assert(pData);
    for (i = 0; i < uSize; i++) {
        crc32 = ((crc32) >> 8) ^ crc32tab[(pData[i]) ^ ((crc32)&0x000000FF)];
    }
}

static uint32_t crc32Finish(void)
{
    crc32 = ~(crc32);
    return crc32;
}
