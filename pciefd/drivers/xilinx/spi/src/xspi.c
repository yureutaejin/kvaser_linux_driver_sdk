...
#define TRACE_TX	0	// Trace sent bytes in ISR
#define TRACE_RX	0	// Trace received bytes in ISR
...
*
* @return
* 		- XST_SUCCESS if the device is successfully stopped.
...
#if TRACE_TX
			pr_debug("Tx: 0x%02X (%u/%u)\n", (u8)Data,
			         InstancePtr->RequestedBytes - InstancePtr->RemainingBytes + 1,
...
#if TRACE_TX
					pr_debug("Tx: 0x%02X (%u/%u)\n", (u8)Data,
					         SpiPtr->RequestedBytes - SpiPtr->RemainingBytes + 1,
							 SpiPtr->RequestedBytes);
#endif
				} else if (DataWidth ==
						XSP_DATAWIDTH_HALF_WORD) {
